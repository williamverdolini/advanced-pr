...
**Menzioni.** Azure DevOps memorizza una menzione nel testo del commento come **`@<GUID>` in maiuscolo**, dove il GUID è l'identity id (lo stesso valore di `author.id`, lì minuscolo): il confronto va fatto case-insensitive. Attenzione a una trappola verificata su un payload reale: **`thread.identities` arriva `null` anche quando il thread contiene una menzione**, quindi la risoluzione dei nomi non può appoggiarsi a quel campo e la directory va costruita dal client.

La ricerca delle persone usa il servizio identità **dell'host** (`ms.vss-features.identity-service`, via `SDK.getService`), lo stesso del picker nativo. È la scelta decisiva sul piano dei permessi: **l'esecuzione avviene nella pagina host, quindi la ricerca sull'intera organizzazione non richiede `vso.identity` né `vso.graph`** e non obbliga l'amministratore a ri-autorizzare l'installazione. Verificato in org di test.

**Verificato end-to-end in org di test:** una menzione scritta dall'estensione produce la notifica di Azure DevOps, con i nomi risolti lato server. Un destinatario che non ha accesso al progetto (access level Basic, mai entrato) genera una mail *"the identity does not have sufficient permissions"* all'autore del commento: è la validazione del prodotto sull'account, identica a quella della UI nativa, e il commento resta salvato con la menzione.

Il typeahead vive dentro una view zone, quindi ne eredita i due vincoli (§7.4): i tasti di navigazione sono gestiti con un **listener nativo sulla textarea** — che gira in fase target, prima che il contenitore fermi l'evento — e la tendina è resa in un **portal su `document.body`**, perché la zona ritaglia il proprio contenuto. La selezione avviene su `mousedown` con `preventDefault`: un `click` toglierebbe prima il fuoco al campo, chiudendo la tendina prima che la scelta arrivi.
